...
const App = (() => {
  ...
  async function processPdf(file, lastKhataNo) {
    ...
    const khatoniParserState = KhatoniTableParser.createState(); // persists khataNo/category carry-forward ACROSS pages of this one file
    ...
    for (let p = 1; p <= numPages; p++) {
      ...
      KhatoniTableParser.parsePageIntoKhatas(table, headerParse.metadata, state.khatas, source, khatoniParserState);
  ...
  async function processImage(file, lastKhataNo) {
    ...
    const khatoniParserState = KhatoniTableParser.createState(); // a single image is a single "file", but keep the same shape as processPdf
    ...
    KhatoniTableParser.parsePageIntoKhatas(built.table, headerParse.metadata, state.khatas, source, khatoniParserState);
    ...
    Object.values(state.khatas).forEach(khata => {
      (khata.plots || []).forEach(plot => {
        ...
        state.allPlotsFlat.push({
          ...
          khasraSource: plot.khasraSource
        });

        const sharedTag = plot.khasraSource === 'merged-cell-carried-forward'
          ? ' <span class="badge badge--muted" title="यह प्लॉट कई खातेदारों के बीच साझा है — दस्तावेज़ में खसरा/क्षेत्रफल केवल पहली पंक्ति पर एक बार छपा है">साझा प्लॉट</span>'
          : '';
        const noPlotTag = plot.noPlotInfo
          ? ' <span class="badge badge--warn" title="'+escapeHtml(plot.note||'')+'">खसरा नहीं दिया गया</span>'
          : '';
        const plotNoDisplay = plot.plotNo ? escapeHtml(plot.plotNo) + sharedTag : '<span class="muted">उपलब्ध नहीं</span>' + noPlotTag;
        ...
        tr.innerHTML = `
          ...
          <td><span class="${statusBadgeClass(plot.needsVerification ? 'Needs Verification' : 'Calculated')}">${plot.needsVerification ? 'सत्यापन आवश्यक' : 'ठीक'}</span></td>
        `;
        tbody.appendChild(tr);
      });
    });

    // Honest cross-check: when a Khata's owner count is well beyond its
    // own योग-stated khasra count, some owners almost certainly have no
    // individually-printed plot at all (a real pattern seen in large
    // joint/ancestral Khatas) — flag it plainly rather than silently
    // guessing which plot each extra owner belongs to.
    const imbalanceBox = el('ownerKhasraImbalanceBox');
    imbalanceBox.innerHTML = '';
    Object.values(state.khatas).forEach(khata => {
      const holderCount = Object.keys(khata.holders).length;
      if (khata.totalKhasraCount != null && holderCount > khata.totalKhasraCount * 2 && holderCount - khata.totalKhasraCount >= 3) {
        const div = document.createElement('div');
        div.className = 'file-chip file-chip--error';
        div.textContent = `⚠ खाता ${khata.khataNo}: ${holderCount} खातेदार दर्ज हैं पर योग सिर्फ़ ${khata.totalKhasraCount} खसरे बताता है — कुछ खातेदारों का प्लॉट/खसरा दस्तावेज़ में अलग से नहीं दिया गया, हर पंक्ति मूल स्कैन से ज़रूर मिलाएं।`;
        imbalanceBox.appendChild(div);
      }
    });
